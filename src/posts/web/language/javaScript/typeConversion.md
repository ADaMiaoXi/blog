...
> 参考答案：
>
> 类型转换可以分为两种，**隐式转换**和**显式转换**。
>
> **1. 隐式转换**
>
> 当不同数据类型之间进行相互运算，或者当对非布尔类型的数据求布尔值的时候，会发生隐式转换。
>
> 预期为数字的时候：算术运算的时候，结果和运算的数都是数字，数据会转换为数字来进行计算。
>
> | 类型      | 转换前    | 转换后 |
> | --------- | --------- | ------ |
> | number    | 4         | 4      |
...
> **2. 显式转换**
>
> 所谓显式转换，就是指程序员强制将一种类型转换为另外一种类型。显式转换往往会使用到一些转换方法。常见的转换方法如下：
>
> - 转换为数值类型：`Number()`，`parseInt()`，`parseFloat()`
> - 转换为布尔类型：`Boolean()`
> - 转换为字符串类型：`toString()`，`String()`
>
> 当然，除了使用上面的转换方法，也可以通过一些快捷方式来进行数据类型的显式转换，如下：
>
