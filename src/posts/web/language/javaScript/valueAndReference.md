...
这来源于 JavaScript 从第一个版本开始时的一个 bug，并且这个 bug 无法被修复。因为修复会破坏现有的代码。

具体原因是因为不同的对象在底层都表现为二进制，在 JavaScript 中二进制前三位都为 0 的话会被判断为 `object` 类型，`null` 的二进制全部为 0，自然前三位也是 0，所以执行 `typeof` 值会返回 `object`。

另外，当打印 `null == undefined` 的时候，返回的是 `true`，这也是面试时经常会被问到的一个问题。

这两个值都表示"无"的意思。

通常情况下，当试图访问某个不存在的或者没有赋值的变量时，就会得到一个 `undefined` 值。JavaScript 会自动将声明但没有进行初始化的变量设为 `undefined`。

而 `null` 值表示空，`null` 不能通过 JavaScript 来自动赋值，也就是说必须要手动给某个变量赋值为 `null`。
...
但是，JavaScript 的设计者觉得这样做还不够，主要有以下两个原因。
...
引用数据类型支持动态对象属性，因为可以定义对象，然后创建引用，再更新对象，并且所有指向该对象的变量都会获得更新。
...
最后说一下关于变量的赋值，其实是可以分为直接赋值和引用赋值的。直接赋值，是指将原始数据类型赋值给变量，而引用赋值是指将一个引用数据类型的引用赋值给变量，这个引用指向堆区实际存在的数据。
...
- JavaScript 的基本数据类型有哪些？基本数据类型和引用数据类型的区别
